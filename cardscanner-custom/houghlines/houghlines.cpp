...
static image_int8u_p new_image_int8u_ptr(unsigned int xsize, unsigned int ysize,
                                         unsigned char *data) {
  image_int8u_p image = NULL;
...
static image_int8u_p gaussian_sampler_byte_bbox(image_int8u_p in,
                                                boundingbox_t bbox,
                                                pixel_float_t scale,
                                                float sigma_scale) {
  ...
  // prec = 3.f;
  _h = (unsigned int)ceil(
      sigma.v * 3.71692219f); //  sqrt(prec * 4.605170186f)); //log(10)*2
  _w = (unsigned int)ceil(
      sigma.u * 3.71692219f); //  sqrt(prec * 4.605170186f)); //log(10)*2
  ...
  for (y = 0; y < out->ysize; y++) {
    ...
    for (x = 0; x < out->xsize; x++) {
      ...
      for (i = 0; i < kernel_y->dim; i++) {
        ...
        while (j < 0)
          j += float_y_size;
        while (j >= float_y_size)
          j -= float_y_size;
        if (j >= (int)bbox.height)
          j = float_y_size - 1 - j;
...
static void InverseBoundingBoxLines(boundingbox_t bbox,
...
static void InverseGaussianSamplerLines(pixel_float_t gs_scale,
                                        std::vector<line_float_t> &lines) {
  ...
  } else if ((gs_scale.u == 1.f) && (gs_scale.v != 1.f)) {
    for (k = 0; k < nsize; k++) {
      lines[k].starty *= iscale_v;
      lines[k].endy *= iscale_v;
    }
...
static short *sobel_edge(unsigned char *src, int w, int h,
                         ORIENT_CODE oriention) {
...
static void _mcv_Canny(unsigned char *_src, unsigned char *_dst, int w, int h,
  ...
  unsigned char *buffer =
      new unsigned char[(w + 2) * (h + 2) + cn * mapstep * 3 * sizeof(int)];
...
static void canny_detector(unsigned char *_src, unsigned char *_dst, int w,
                           int h, double low_thresh, double high_thresh) {
  _mcv_Canny(_src, _dst, w, h, low_thresh, high_thresh);
}
//*******************************************************************************
//*******************************************************************************
// standard hough
...
static void _mcv_HoughLinesStandard(unsigned char *image, int w, int h,
                                    float rho, float theta, int threshold,
                                    int linesMax, double min_theta,
                                    double max_theta,
                                    std::vector<vec_float_t> &lines) {
  int i, j;
  float irho = 1 / rho;
  int step = w;
  int offy;
  ...
  for (i = 0; i < linesMax; i++) {
    ...
    vec_float_t v = {line.rho, line.angle};
    lines.push_back(v);
  }
...
// max_theta must be greater than min_theta
static void hough_line_standard(unsigned char *image, int w, int h, double rho,
                                double theta, int threshold, double min_theta,
                                double max_theta,
                                std::vector<line_float_t> &lines) {
  int k;
  float d;
  ...
  _mcv_HoughLinesStandard(image, w, h, (float)rho, (float)theta, threshold,
                          INT_MAX, min_theta, max_theta, _lines);
...
static void _mcv_HoughLinesProbabilistic(int w, int h, unsigned char *_image,
                                         float rho, float theta, int threshold,
                                         int lineLength, int lineGap,
                                         int linesMax,
                                         std::vector<line_float_t> &_lines) {
  int i, j;
  float irho = 1 / rho;
  int offset, offy;
  ...
  for (; count > 0; count--) {
    ...
    if (good_line) {
      line_float_t _lr = {(float)line_end[0].x, (float)line_end[0].y,
                          (float)line_end[1].x, (float)line_end[1].y};
...
static void hough_line_probabilistic(unsigned char *image, int w, int h,
                                     ...
                                     std::vector<line_float_t> &lines) {
  _mcv_HoughLinesProbabilistic(w, h, image, (float)rho, (float)theta, threshold,
                               (int)round(minLineLength), (int)round(maxGap),
                               INT_MAX, lines);
}
...
static void _hough_line_detector(unsigned char *src, int w, int h, float scaleX,
                                 ...
                                 HOUGH_LINE_TYPE_CODE _type, boundingbox_t bbox,
                                 std::vector<line_float_t> &lines) {
  ...
  case HOUGH_LINE_STANDARD:
    hough_line_standard(edge, sub_src->xsize, sub_src->ysize, (double)hough_rho,
                        (double)hough_theta, hough_thresh,
  ...
  case HOUGH_LINE_PROBABILISTIC:
    hough_line_probabilistic(edge, sub_src->xsize, sub_src->ysize,
                             (double)hough_rho, (double)hough_theta,
                             hough_thresh, (double)min_theta_linelength,
                             (double)max_theta_gap, lines);
...
int HoughLineDetector(unsigned char *src, int w, int h, float scaleX,
  ...
  if (bbox.x < 0 || bbox.x > w - 1 || bbox.y < 0 || bbox.y > h - 1 ||
      bbox.width <= 0 || bbox.height <= 0 || bbox.x + bbox.width > w ||
